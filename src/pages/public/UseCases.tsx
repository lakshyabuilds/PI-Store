    ...
    <div className="max-w-7xl mx-auto px-6 py-24 md:py-32">
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        {avatars.map((opt, i) => (
          ...
          >
            <div
              className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-6 bg-bg-surface`}
            >
              {opt.icon}
            </div>
            ...
            <p className="text-text-main font-bold mb-8 opacity-80">
              {opt.subtitle}
            </p>
            ...
            <ul className="space-y-4 mb-8 flex-1">
              {opt.points.map((point, j) => (
                <li
                  key={j}
                  className="flex gap-3 text-text-muted font-medium leading-relaxed"
                >
      ...
      <div className="mt-24 text-center">
        <Link
          to="/register"
          className="neo-button inline-flex items-center justify-center gap-3 w-full sm:w-auto px-6 sm:px-10 py-4 sm:py-5 bg-text-main text-bg-base font-black text-lg sm:text-xl rounded-2xl hover:bg-primary hover:text-black transition-colors shadow-xl"
        >
          Start Selling Today
        </Link>
      </div>
